...
Add-BuildTask AppVeyorPublish -If ($env:APPVEYOR) {
	...
	if($env:APPVEYOR_REPO_BRANCH -like 'master'){
		...
		Try {
			...
			Write-Build Cyan "$ModuleName PowerShell Module version $env:version published to the PowerShell Gallery."
		} Catch {
			Write-Warning "Publishing update $env:version to the PowerShell Gallery failed."
			throw $_
		}
	} else {
		Write-Build Cyan 'Only the master branch gets published'
	}
}
